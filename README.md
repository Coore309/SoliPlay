...
2. 双击根目录下的 `start.bat` 生成快捷方式（若无法生成，请检查文件夹读写权限，或以管理员身份运行）后，双击同目录下生成的 `SoliPlay` 快捷方式启动（或直接运行 `SoliPlay.exe`），该快捷方式可以拖动到桌面上进行使用（ `.exe` 则不行）。
3. 首次使用需在设置页面填入 DeepSeek API Key，请自行前往 DeepSeek 开放平台获取。
4. 浏览器访问 `http://localhost:8080` 使用故事机（现已支持自动弹出网页）。
5. 若提醒故事创建失败或导演格式错误，请适当调整提示词或回复后重试。

## 主要特性

- AI 导演控制剧情走向，调度多个 AI 演员扮演角色
- 支持跑团（TRPG）和小说（Novel）两种模式
- D100 判定系统，小说模式可触发“奇迹”
- 角色认知系统，称呼随主角感知变化（目前不全面）
- 故事自动保存/加载，支持多故事管理
- 头像管理（裁剪、文件夹分类、与角色绑定）
...
- 头像库支持上传、裁剪、移动和删除，可绑定给角色（绑定后需要继续对话一、两回合后才能将绑定结果保存至本地，待优化）。
